...
from LSTM_CRF_PA_SL import LSTM_CRF_PA_SL
...
X_char_merge_train, y_merge_train, sign_merge_train = utils.merge_export_and_PA_train_data_with_sign(X_char_train, y_train, X_char_PA, y_PA)
...
with tf.Session(config=cpu_config) as sess:
    ...
    with tf.variable_scope("model", reuse=None, initializer=initializer):
        model = LSTM_CRF_PA_SL(num_chars=num_chars, num_classes=num_classes, num_steps=num_steps, num_epochs=num_epochs, embedding_matrix=embedding_matrix)
    print "training model"
    sess.run(tf.global_variables_initializer())
    model.train(sess, X_char_merge_train, y_merge_train, sign_merge_train, X_char_train, y_train, X_char_dev, y_dev, X_char_test, y_test)
